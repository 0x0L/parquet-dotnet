...
namespace Snappy.Sharp
{
    ...
    {
        const int MAX_INCREMENT_COPY_OVERFLOW = 10;
        ...
        */
        static int[] DecompressTagSlow(
            byte[] input, int ipIndex, byte[] output, int outputLimit, int outputOffset, int opIndex)
        {
            // read the op code
            byte opCode = input[ipIndex++];
            ushort entry = opLookupTable[opCode];
        ...
        static void CopyLiteral(byte[] input, int ipIndex, byte[] output, int opIndex, int length)
        {
        ...
        static void CopyCopy(byte[] output, int length, int opIndex, int outputLimit, int copyOffset)
        {
        ...
        * or memmove().
        */
        static void IncrementalCopy(byte[] source, int srcIndex, byte[] output, int opIndex, int length)
        {
        ...
        static void IncrementalCopyFastPath(byte[] output, int srcIndex, int opIndex, int length)
        {
            int copiedLength = 0;
        ...
        static readonly ushort[] opLookupTable = {
