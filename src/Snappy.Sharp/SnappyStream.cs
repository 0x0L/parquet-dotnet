...
namespace Snappy.Sharp
{
    ...
    class SnappyStream : Stream
    {
        ...
        static readonly byte[] StreamHeader = { (byte)'s', (byte)'N', (byte)'a', (byte)'P', (byte)'p', (byte)'Y' };
        const byte StreamIdentifier = 0xff;
        const byte CompressedType = 0x00;
        const byte UncompressedType = 0x01;

        // allocate a 64kB buffer for the (de)compressor to use
        readonly byte[] internalBuffer = new byte[1 << (BLOCK_LOG + 1)];
        int internalBufferIndex;
        int internalBufferLength;

        readonly SnappyCompressor compressor;
        readonly SnappyDecompressor decompressor;
        ...
        int ProcessRemainingInternalBuffer(byte[] buffer, int offset, int count)
        {
        ...
        int GetChunkUncompressedLength()
        {
